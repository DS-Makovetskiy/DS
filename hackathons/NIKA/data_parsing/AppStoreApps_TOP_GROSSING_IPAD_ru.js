...
const fetchData = async () => {
  ...
  const saveDataToFile = async (apps, index) => {
    const filePath = `../data/AppStoreApps_TOP_GROSSING_IPAD_ru${String(index).padStart(3, '0')}.json`;
    ...
    for (let page = 0; page < 50; page++) {  // 50 страниц по 200 элементов = 10000 приложений
      try {
        const apps = await store.list({
          category: category,
          collection: store.collection.TOP_GROSSING_IPAD,
...
// const fetchData = async () => {
//   const filePath = '../data/AppStoreApps_TOP_GROSSING_IPAD_ru.json';
...
//   fs.writeFileSync('AppStoreApps_TOP_GROSSING_IPAD_ru.json', JSON.stringify(allApps, null, 2));
// };
